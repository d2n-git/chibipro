...
class InSertPictureModel extends Model
{
    ...
    {
        $db = \Config\Database::connect();
        $sql="SELECT pt.idPictures, pt.Name, date_format(cp.DateExpiry,'%Y-%m-%d') as DateExpiry, pt.StandarPrice, pt.PriceOfUser, pt.MaxPrice, cp.Price, pt.ExtraDetail, cp.Note FROM pictures pt left join confirmofpainter cp ON pt.idPictures = cp.idPicture WHERE idPictures = ? group by pt.idPictures";
